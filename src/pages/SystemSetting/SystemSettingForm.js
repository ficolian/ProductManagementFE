...
import { ApiResponseStatusUnauthorized, ApiResponseSuccess, EmptyString, FormTypeAdd, FormTypeEdit, GuidDefault, Undefined } from "utils/Constant";
import FailModal from "components/modal/FailModal";
import LoadingModal from "components/modal/LoadingModal";
import SuccessModal from "components/modal/SuccessModal";
import ConfirmationModal from "components/modal/ConfirmationModal";
import { ApiGetSystemSettingById, ApiDeleteSystemSetting, ApiInsertSystemSetting, ApiUpdateSystemSetting } from "utils/constants/api";


export default function SystemSettingForm(){
    let navigate = useNavigate()

    const{ fetch: insertSystemSetting } = useFetch(ApiInsertSystemSetting)
    const{ fetch: updateSystemSetting } = useFetch(ApiUpdateSystemSetting)
    const{ fetch: deleteSystemSetting } = useFetch(ApiDeleteSystemSetting)
    const{ fetch: getSystemSettingById } = useFetch(ApiGetSystemSettingById)
    ...
    useEffect(() => {
        if(formType === FormTypeAdd){
            ...
            Promise.all([
                fGetSystemSettingById(paramKostId, paramSystemId)
            ]).then(() => {
                setLoading(false)
            })
    ...
    function fAddTableRow(){
        setTableRow([
            ...tableRow,
            { systemText: EmptyString, systemValue: EmptyString, description: EmptyString },  // New row with empty data
        ]);
    }
    ...
    function fSaveBtnClick(){
        if(fFormValidation()){
            setConfirmationModal({ isOpen: true, data: { onSuccess: async () => { 
                ...
                if(formType === FormTypeAdd){
                    fInsertSystemSetting() 
                }else{
                    fUpdateSystemSetting()
                }
            }}})
        }
    }
    ...
    function fInsertSystemSetting(){
        ...
        for(let i = 0; i < tableRow.length; i++){
            let datas = {
                kostId: GuidDefault,
                systemId: systemId,
                systemText: tableRow[i].systemText,
                systemValue: tableRow[i].systemValue,
        ...
        insertSystemSetting({
            ...
            if(response.status === ApiResponseSuccess){
                setSuccessModal({ isOpen: true, data: { onSuccess: async () => { navigate("/setting/system-setting") }} })
    ...
    async function fDeleteSystemSetting(systemId){
        setLoading(true)
    ...
    async function fGetSystemSettingById(kostId, systemId){
        console.log(kostId)
        await getSystemSettingById({
            data: {
                kostId: kostId === Undefined ? GuidDefault : kostId,
                systemId: systemId
            }
        }).then((response) => {
            if(response.status === ApiResponseSuccess){
    ...
    return(
        <div className="flex flex-col gap-2">
            <div>
                <span className="font-semibold text-2xl">System Setting Detail - {formType}</span>
            </div>
            <hr />
                        ...
                        { tableRow?.map((data, i) => {
                            return(
                                <tr className=" row min-h-14 border-y" key={i}>
                                    ...
                                    <td className="p-2">
                                        <input className="input" value={data.systemText} onChange={(e) => fHandleChange(e, i, 'systemText')} /> 
                                        <span className="text-red-500 hidden">System text is required!</span>
                                    </td>
                                    <td className="p-2">
                                        <input className="input" value={data.systemValue} onChange={(e) => fHandleChange(e, i, 'systemValue')} /> 
            ...
            <div className="mt-5 flex justify-end gap-2">
                <BackButton onClick={() => { navigate("/setting/system-setting") }} />
